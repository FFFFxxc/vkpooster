...
function addButton(postEl) {
  ...
  // Repost button - фиолетовый градиент
  ...
  btnRepost.style.cssText = baseStyles + `
    background: linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%);
    color: white;
    box-shadow: 0 2px 8px rgba(99, 102, 241, 0.4);
  `;
  
  btnRepost.addEventListener('mouseenter', () => {
    btnRepost.style.transform = 'translateY(-2px)';
    btnRepost.style.boxShadow = '0 4px 16px rgba(99, 102, 241, 0.5)';
  });
  btnRepost.addEventListener('mouseleave', () => {
    btnRepost.style.transform = 'translateY(0)';
    btnRepost.style.boxShadow = '0 2px 8px rgba(99, 102, 241, 0.4)';
  });
  ...
  btnRepost.addEventListener('click', async (event) => {
    ...
    if (!postUrl) return showCustomAlert('Не удалось определить ссылку на пост.');
    await openModal(postUrl);
  });
  actions.appendChild(btnRepost);

  // Comment button - зелёный градиент
  const btnComment = document.createElement('button');
  btnComment.type = 'button';
  btnComment.className = `${BUTTON_CLASS} vkr-btn-comment`;
  btnComment.innerHTML = '💬 Коммент';
  btnComment.style.cssText = baseStyles + `
    background: linear-gradient(135deg, #22c55e 0%, #16a34a 100%);
    color: white;
    box-shadow: 0 2px 8px rgba(34, 197, 94, 0.4);
  `;
  
  btnComment.addEventListener('mouseenter', () => {
    btnComment.style.transform = 'translateY(-2px)';
    btnComment.style.boxShadow = '0 4px 16px rgba(34, 197, 94, 0.5)';
  });
  btnComment.addEventListener('mouseleave', () => {
    btnComment.style.transform = 'translateY(0)';
    btnComment.style.boxShadow = '0 2px 8px rgba(34, 197, 94, 0.4)';
  });
...
function processComments(root = document) {
  ...
  comments.forEach(comment => {
    try {
      ...
      // Красивые стили для кнопки накрутки
      btn.style.cssText = `
      ...
      btn.addEventListener('mouseenter', () => {
        ...
        btn.style.boxShadow = '0 4px 12px rgba(245, 158, 11, 0.6)';
      });
      btn.addEventListener('mouseleave', () => {
        btn.style.transform = 'scale(1)';
        btn.style.boxShadow = '0 2px 8px rgba(245, 158, 11, 0.4)';
      });
      ...
      btn.onclick = async (e) => {
        ...
        try {
          ...
          if (res.ok) {
            ...
            btn.style.boxShadow = '0 2px 8px rgba(239, 68, 68, 0.5)';
          } else {
...
function showToast(text, type = 'info') {
  // Create toast in main document with fixed position
  const existingToast = document.querySelector('.vkr-toast-notify');
  if (existingToast) existingToast.remove();
  
  const toast = document.createElement('div');
  toast.className = 'vkr-toast-notify';
  
  const colors = {
    success: { bg: 'linear-gradient(135deg, rgba(34, 197, 94, 0.95), rgba(22, 163, 74, 0.95))', border: '#22c55e' },
    error: { bg: 'linear-gradient(135deg, rgba(239, 68, 68, 0.95), rgba(220, 38, 38, 0.95))', border: '#ef4444' },
    info: { bg: 'linear-gradient(135deg, rgba(99, 102, 241, 0.95), rgba(139, 92, 246, 0.95))', border: '#6366f1' }
  };
...
function bindModalEvents() {
  ...
  modalEl.querySelectorAll('.vkr-mode-btn[data-mode]').forEach(btn => {
    btn.onclick = () => {
      ...
      modalEl.querySelectorAll('.vkr-mode-btn[data-mode]').forEach(b => b.classList.remove('active'));
  ...
  async function loadHiddenGroups() {
    ...
    if (hidden.length === 0) {
      hiddenList.innerHTML = '<div style="text-align:center;color:#64748b;padding:20px">Нет скрытых групп</div>';
...
async function loadPost(postUrl) {
  ...
  try {
    const response = await Promise.race([
      ...
      new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout 15 сек')), 15000))
    ]);
    ...
    if (post.attachments) {
      ...
      html += '<div style="color:#6366f1;margin-top:8px">' + types.join(' ') + '</div>';
    }
...
async function sendToGroups() {
  ...
  try {
    ...
    if (!fail) {
      setStatus(`✅ Отправлено в ${ok} групп!`, 'success');
...
function renderPhrases(phrases) {
  ...
  if (phrases.length === 0) {
    list.innerHTML = '<div style="color: #64748b; text-align: center; padding: 10px; font-size: 13px;">Нет заготовленных фраз</div>';
    return;
  }
  
  phrases.forEach((text, i) => {
    const div = document.createElement('div');
    div.style.cssText = 'display:flex; justify-content: space-between; align-items: center; background: rgba(255,255,255,0.05); padding: 8px 12px; border-radius: 8px; font-size: 13px; margin-bottom: 6px;';
    div.innerHTML = `
...
function bindCommentModalEvents() {
  ...
  $('vkr-send-comment-btn').onclick = async () => {
    ...
    if (tpl.length === 0) return showCustomAlert('❌ Добавьте хотя бы одну фразу!');
    ...
    try {
      ...
      if (res.ok) {
        $('vkr-comment-status').innerHTML = `<span style="color:#22c55e">✅ Отправлено: "${randomText}"</span>`;
        setTimeout(() => {
...
function createBestPostsModal() {
  const html = `
<div id="vkr-best-posts-overlay" style="display:none; z-index: 999999; pointer-events: auto; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.75); align-items: center; justify-content: center; backdrop-filter: blur(8px);">
  <div id="vkr-modal" style="max-width: 700px;">
    <div id="vkr-bp-search-view">
      <div id="vkr-modal-header">
        <span>🔥 Лучшие посты</span>
        <button class="vkr-bp-close" style="background: none; border: none; color: #94a3b8; font-size: 24px; cursor: pointer;">✕</button>
      </div>
      <div style="padding: 20px; overflow-y: auto; color: #f8fafc;">
        <div class="vkr-section" style="margin-bottom: 16px;">
          <div class="vkr-section-title">📌 Паблик</div>
          <div style="color: #6366f1;">ID: <span id="vkr-bp-owner-id">-</span></div>
        </div>
        <div class="vkr-section" style="margin-bottom: 16px;">
          <div class="vkr-section-title">📅 Период</div>
          <div style="display: flex; gap: 12px; margin-bottom: 12px;">
            <div style="flex: 1; display: flex; align-items: center; gap: 8px;"><label>С:</label><input type="text" id="vkr-bp-date-from" placeholder="ДД.ММ.ГГГГ" maxlength="10" style="flex: 1; padding: 10px; background: #1a1a25; border: 1px solid rgba(255,255,255,0.08); border-radius: 10px; color: #f8fafc;"></div>
            <div style="flex: 1; display: flex; align-items: center; gap: 8px;"><label>По:</label><input type="text" id="vkr-bp-date-to" placeholder="ДД.ММ.ГГГГ" maxlength="10" style="flex: 1; padding: 10px; background: #1a1a25; border: 1px solid rgba(255,255,255,0.08); border-radius: 10px; color: #f8fafc;"></div>
          </div>
          <div style="display: flex; flex-wrap: wrap; gap: 8px;">
            <button class="vkr-mode-btn" data-days="7" style="padding: 6px 12px; font-size: 12px;">Неделя</button>
            <button class="vkr-mode-btn" data-days="30" style="padding: 6px 12px; font-size: 12px;">Месяц</button>
            <button class="vkr-mode-btn" data-days="90" style="padding: 6px 12px; font-size: 12px;">3 месяца</button>
            <button class="vkr-mode-btn" data-days="180" style="padding: 6px 12px; font-size: 12px;">6 месяцев</button>
            <button class="vkr-mode-btn" data-days="365" style="padding: 6px 12px; font-size: 12px;">Год</button>
            <button class="vkr-mode-btn" data-days="3650" style="padding: 6px 12px; font-size: 12px;">Всё время</button>
          </div>
        </div>
        <div class="vkr-section" style="margin-bottom: 16px;">
          <div class="vkr-section-title">📊 Сортировка</div>
          <div style="display: flex; gap: 8px; margin-bottom: 12px; flex-wrap: wrap;">
            <button class="vkr-mode-btn active" data-metric="likes" style="padding: 8px 16px;">❤️ Лайки</button>
            <button class="vkr-mode-btn" data-metric="views" style="padding: 8px 16px;">👁 Просмотры</button>
            <button class="vkr-mode-btn" data-metric="reposts" style="padding: 8px 16px;">🔄 Репосты</button>
            <button class="vkr-mode-btn" data-metric="comments" style="padding: 8px 16px;">💬 Комменты</button>
          </div>
          <div style="display: flex; align-items: center; gap: 8px;"><label>Мин. значение:</label><input type="number" id="vkr-bp-min-value" placeholder="0" min="0" style="flex: 1; padding: 10px; background: #1a1a25; border: 1px solid rgba(255,255,255,0.08); border-radius: 10px; color: #f8fafc;"></div>
        </div>
        <div class="vkr-section" style="margin-bottom: 16px;">
          <div class="vkr-section-title">📦 Загрузить постов</div>
          <input type="number" id="vkr-bp-count" value="50" min="10" max="300" step="10" style="width: 100%; padding: 10px; background: #1a1a25; border: 1px solid rgba(255,255,255,0.08); border-radius: 10px; color: #f8fafc;">
        </div>
        <div class="vkr-section">
          <label class="vkr-checkbox"><input type="checkbox" id="vkr-bp-only-photo"> Только с фото</label>
          <label class="vkr-checkbox"><input type="checkbox" id="vkr-bp-only-video"> Только с видео</label>
          <label class="vkr-checkbox"><input type="checkbox" id="vkr-bp-no-reposts"> Без репостов</label>
        </div>
      </div>
      <div style="padding: 16px 20px; border-top: 1px solid rgba(255,255,255,0.08); display: flex; justify-content: center;">
        <button id="vkr-bp-search" class="vkr-mode-btn active" style="padding: 12px 24px; font-size: 14px;">🔍 Найти лучшие посты</button>
      </div>
    </div>
    
    <div id="vkr-bp-results-view" style="display:none; color: #f8fafc;">
      <div id="vkr-modal-header">
        <span>🔥 Результаты</span>
        <button class="vkr-bp-close" style="background: none; border: none; color: #94a3b8; font-size: 24px; cursor: pointer;">✕</button>
      </div>
      <div style="padding: 16px 20px; border-bottom: 1px solid rgba(255,255,255,0.08); display: flex; align-items: center; justify-content: space-between; gap: 12px;">
      ...
      <div id="vkr-bp-progress" style="display:none; padding: 20px; text-align: center;">
        ...
        <div id="vkr-progress-bar" style="width: 100%; height: 8px; background: #1a1a25; border-radius: 4px; overflow: hidden;"><div style="height: 100%; background: linear-gradient(90deg, #6366f1, #8b5cf6); width: 0%;"></div></div>
        <div id="vkr-bp-progress-info" style="margin-top: 12px;">Загружено: 0</div>
      </div>
...
function bindBestPostsEvents() {
  ...
  bestPostsModalEl.querySelectorAll('[data-metric]').forEach(btn => {
    btn.onclick = () => {
      bestPostsModalEl.querySelectorAll('[data-metric]').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
    };
  });
...
async function searchBestPosts() {
  ...
  const settings = {
    ...
    metric: bestPostsModalEl.querySelector('[data-metric].active')?.dataset.metric || 'likes',
  ...
  if (!dateFrom || !dateTo) return showCustomAlert('❌ Укажите корректные даты');
  if (dateFrom > dateTo) return showCustomAlert('❌ Дата "С" должна быть раньше даты "По"');
  ...
  try {
    while (isLoadingPosts) {
      ...
      bestPostsModalEl.querySelector('#vkr-bp-progress #vkr-progress-bar div').style.width = progress + '%';
      $('vkr-bp-progress-info').textContent = `Загружено: ${loadedPosts.length} постов`;
...
function renderResults(posts, metric) {
  ...
  if (posts.length === 0) {
    list.innerHTML = '<div style="text-align: center; color: #64748b;">Постов не найдено</div>';
    return;
  }
  ...
  posts.forEach((post, index) => {
    ...
    if (post.attachments) {
      ...
      if (photo) {
        ...
        if (thumb) {
          photoHtml = `<img src="${thumb.url}" style="width: 80px; height: 80px; object-fit: cover; border-radius: 10px; flex-shrink: 0;">`;
        }
      }
    }
    
    html += `
      <div style="background: #1a1a25; border: 1px solid ${index < 3 ? 'rgba(245, 158, 11, 0.5)' : 'rgba(255,255,255,0.08)'}; border-radius: 14px; padding: 16px; margin-bottom: 16px;">
        <div style="display: flex; justify-content: space-between; margin-bottom: 12px;">
          <span style="color: #f8fafc; font-weight: 600;">#${index + 1}</span>
          <span style="color: #64748b; font-size: 13px;">📅 ${formatDate(date)}</span>
        </div>
        <div style="display: flex; gap: 12px; margin-bottom: 12px;">
          ${photoHtml}<div style="color: #94a3b8; font-size: 14px; line-height: 1.5; flex: 1;">${esc(text)}</div>
        </div>
        <div style="display: flex; gap: 16px; margin-bottom: 12px; padding: 8px 0; border-top: 1px solid rgba(255,255,255,0.08); border-bottom: 1px solid rgba(255,255,255,0.08); color: #94a3b8; font-size: 13px;">
        ...
        <div style="display: flex; gap: 8px;">
          <button class="vkr-mode-btn active vkr-bp-post-repost" data-post-url="${postUrl}" style="flex: 1;">📋 Репост</button>
          <a href="${postUrl}" target="_blank" class="vkr-mode-btn" style="background: transparent; border: 1px solid rgba(255,255,255,0.08); text-decoration: none; display: flex; align-items: center; justify-content: center;">🔗 Открыть</a>
        </div>
      </div>
    `;
  });
